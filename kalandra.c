...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/05/26 20:52:43 by vbauguen          #+#    #+#             */
/*   Updated: 2016/05/27 14:40:19 by vbauguen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_draw_kalandra(t_id *s)
{
	...
	while (++x < W_X)
	{
		...
		while (++y < W_Y)
		{
			s->z_r = sqrt(fabsl((x * (s->x2 - s->x1) / W_X + s->x1))) * s->zoom;
			s->z_i = sqrt(fabsl((y * (s->y2 - s->y1) / W_Y + s->y1))) * s->zoom;
			s->i = -1;
